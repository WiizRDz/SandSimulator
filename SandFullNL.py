...
while (True):
	...
	class Barrier():
		...
		def ShowBarrier(self):
			...
			board[self.y-1][self.x] = 1
			board[self.y-1][self.x-1] = 1			
			pygame.draw.rect(screen, (r,g,b), (self.x*5-5,self.y*5-5,15,15),0)
	...
	for i in range(0,math.ceil(screen.get_height()/5)-1):
		for c in range(1,math.ceil(screen.get_width()/5)-1):
			if bn[i][c] == 1:
				pygame.draw.rect(screen, cl[dc], (c*5,i*5,5,5),0)
				dc += 1
